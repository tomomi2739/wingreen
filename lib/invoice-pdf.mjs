...
export async function buildInvoicePdf({ company, invoice, client, items, logoBytes }) {
  ...
  textRight(yen(invoice.total), boxTop + 36, R - 18, 22);
  ...
  for (const it of rows) {
    ...
    text(it.name ?? '', top + 10, COL.name + 8, 9.5);
    if (it.quantity !== null) textRight(Number(it.quantity).toLocaleString('ja-JP'), top + 10, COL.qty + 40, 9.5);
    if (it.unitPrice !== null) textRight(yen(it.unitPrice), top + 10, COL.unit + 60, 9.5);
    textRight(yen(it.amount), top + 10, COL.amount - 8, 9.5);
    if (it.note) text(it.note, top + 22, COL.name + 8, 8, SUB);
    top += it.note ? 34 : 24;
